...
이 파일은 Claude Code가 halias 코드베이스를 작업할 때 참고하는 가이드입니다.
...
- **상태**: v0.1.0 npm publish 완료. 실사용 + 발견되는 마찰점 기반으로 점진 개선 중.
...
**모든 코드 변경은 README와 CHANGELOG 갱신을 동반합니다.** 코드만 바꾸고 문서를 안 건드리면 작업이 끝난 게 아닙니다. Claude Code는 어떤 작업이든 다음을 자동으로 검토하고 필요한 갱신을 함께 수행해야 합니다.
...
## Files Claude Code shouldn't touch without confirmation
...
### 완료된 버전
- **v0.2.0** — 컨텍스트 인식 검색 (현재 디렉토리 사용 빈도 정렬)
...
- **v0.7.0** — 1.0 준비: 신뢰 (vitest 테스트 스위트 + CI, store 동시성 lockfile `mutateStore()`, 지원 매트릭스 명시)
- **v1.0.0** — 첫인상 & 선언 (fzf `--preview` 창, vhs 데모 GIF `docs/demo.tape`, README 안정성 약속 — 데이터 포맷 동결 + semver)

### 1.0 이후
- **MCP 서버 (1.x 후보)** — 단축키 + 디렉토리별 사용 통계를 AI 에이전트에게 개인 컨텍스트로 노출. 포지셔닝: "생성은 AI, 보존·머슬 메모리는 halias". AI 시대에 halias 데이터가 에이전트의 입력이 되는 방향.
- 그 외 실 사용자 피드백 누적되면 결정. 데이터 포맷/CLI 는 안정성 약속(README) 준수 — 깨지는 변경은 메이저에서만.
