...
export async function runRemove(name: string | undefined): Promise<void> {
  ...
  if (!target) {
    const selected = await p.select({
      message: '삭제할 단축키를 선택하세요',
      options: store.shortcuts.map((s) => {
        const count = usageMap.get(s.name) ?? 0;
        const cmdHint = s.type === 'alias' ? s.command : '<function>';
        const usageHint = count > 0 ? `  · ${count}회 사용` : '  · 미사용';
        return {
          value: s.name,
          label: s.name,
  ...
  if (!shortcut) {
    console.log(chalk.red(`단축키를 찾을 수 없습니다: ${target}`));
    return;
  }
  ...
  if (count >= FREQUENT_THRESHOLD) {
    const confirmed = await p.confirm({
      message:
        chalk.yellow(`'${target}' 은(는) ${count}회 사용한 자주 쓰는 단축키입니다.`) +
        '\n  ' +
        chalk.dim('정말 삭제할까요?'),
      initialValue: false,
    });
  ...
  if (!removed) {
    console.log(chalk.red(`삭제 실패: ${target}`));
    return;
  }
  const updated = await readStore();
  await generateAliasesFile(updated);

  const usageInfo = count > 0 ? chalk.dim(` (사용 횟수: ${count})`) : '';
  console.log(chalk.green(`✓ ${target} 삭제됨`) + usageInfo);
}
