...
- 테스트 스위트 (단위 + 실제 bash/zsh 통합) CI 에서 실행
- 모든 store 쓰기를 락으로 직렬화 — 셸 간 lost-update 해소
- 지원 매트릭스 문서화

### v1.0.0 — 안정성 선언 ✅

- 퍼지 검색 미리보기 창, 데모 GIF, 위의 안정성 약속

### 다음 버전

- **MCP 서버** — 단축키와 디렉토리별 사용 통계를 AI 코딩 에이전트가 개인 컨텍스트로 조회할 수 있게. AI가 긴 명령을 생성하면 `ha add --last`가 보존하고, 에이전트는 사용자가 어디서 뭘 쓰는지 배웁니다.

그 외에는 체크리스트 채우기보다 **실 사용에서 발견되는 마찰점** 기반으로 진행합니다.
